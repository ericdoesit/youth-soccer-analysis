#!/usr/bin/env python3
"""
Scrape ECNL Regional League Boys standings to build club + team database.
API: https://api.athleteone.com/api/Script/get-conference-standings/{eventId}/{orgId}/{seasonId}/{divisionId}/{standingId}
"""
...
# Include conference standings (0) and all playoff brackets
STANDING_TYPES = {
    0:  "Conference",
    23: "Playoffs-East",
    24: "Playoffs-Central",
...
class StandingsParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "span" and "individual-team-item" in attrs_d.get("class", ""):
            # Flush any pending team (conf_label may never arrive for conf standings)
        ...
        # Conference label lives in a plain <span> (no class) after the team name
    ...
    def finalize(self):
        """Flush any team that never received a conf_label span."""
        if self.current.get("team_name"):
            self.teams.append(dict(self.current))
            self.current = {}


def fetch_standings(event_id, division_id, standing_id):
    url = (
...
def clean_club_name(team_name, age_group):
    """Strip age group suffix from team name to approximate club name."""
    suffix = f" ECNL RL {age_group}"
    if team_name.endswith(suffix):
        return team_name[: -len(suffix)].strip()
    # Try stripping just the age group
    for ag in AGE_GROUPS.values():
        s = f" ECNL RL {ag}"
        if team_name.endswith(s):
            return team_name[: -len(s)].strip()
    return team_name


def main():
    all_teams = []    # every team row
    seen_team_ids = set()

    total = len(CONFERENCES) * len(AGE_GROUPS) * len(STANDING_TYPES)
    done = 0

    for event_id, conf_name in CONFERENCES.items():
        for div_id, age_group in AGE_GROUPS.items():
            for standing_id, standing_name in STANDING_TYPES.items():
                teams = fetch_standings(event_id, div_id, standing_id)
                for t in teams:
                    if t["team_id"] not in seen_team_ids:
                        seen_team_ids.add(t["team_id"])
                        # Use conf_label from the HTML span if available;
                        # fall back to the CONFERENCES dict by event_id, then the call's conf_name
                        actual_conf = (
                            t.get("conf_label")
    ...
    with open(teams_path, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=[
            ...
            "age_group", "conference", "standing", "league", "season"
        ])
        w.writeheader()
